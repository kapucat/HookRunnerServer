...
func accountRegisterHandler(w http.ResponseWriter, r *http.Request) {
	...
	// ログイン名は大文字・小文字を区別しない。
	request.LoginName = strings.ToLower(strings.TrimSpace(request.LoginName))
	...
	if err != nil {
		...
		writeAccountJSON(
			...
			AccountRegisterResponse{Message: "account creation failed"},
		)
		return
	}
...
`,
	...
	if err != nil {
		...
		if errors.As(err, &pqError) && string(pqError.Code) == "23505" {
			writeAccountJSON(
				w,
				http.StatusConflict,
				AccountRegisterResponse{
